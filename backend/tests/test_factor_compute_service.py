...
from app.services.factor_service import FactorComputeService, ComputeResult, DEFAULT_LOOKBACK_DAYS
...
class TestFactorComputeServiceBasics:
    ...
    def service(self, mock_db_client):
        """创建服务实例"""
        with patch('app.services.factor_service.TradingCalendar'):
            with patch('app.services.factor_service.DataConfigLoader'):
                with patch('app.services.factor_service.get_preprocess_loader'):
                    with patch('app.services.factor_service.PipelineFactory'):
    ...
    def test_register_config_tables(self, mock_db_client):
        """测试注册配置表"""
        with patch('app.services.factor_service.TradingCalendar'):
            with patch('app.services.factor_service.DataConfigLoader') as mock_loader:
...
class TestComputeFactorFlow:
    ...
    def test_compute_factor_not_found(self, service):
        ...
        with patch('app.services.factor_service.discover_factors'):
            with patch('app.services.factor_service.get_factor', return_value=None):
...
class TestPreprocessOptions:
    ...
    def service(self):
        ...
        with patch('app.services.factor_service.TradingCalendar'):
            with patch('app.services.factor_service.DataConfigLoader'):
                with patch('app.services.factor_service.get_preprocess_loader') as mock_loader:
...
class TestErrorHandling:
    ...
    def test_compute_factor_exception_handling(self, service):
        """测试计算异常处理"""
        with patch('app.services.factor_service.discover_factors'):
            with patch('app.services.factor_service.get_factor', side_effect=Exception("Test error")):
