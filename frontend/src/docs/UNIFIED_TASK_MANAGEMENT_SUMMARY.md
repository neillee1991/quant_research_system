...
本次重构完整实现了讨论文档中的**选项 B：分层抽象 - 基础层 + 扩展层**方案，覆盖了 Phase 1 和 Phase 2 的全部内容。

## 架构总览

```
通用层（所有任务共用）:
├── config/taskTypes.ts                    # 任务类型配置体系
├── hooks/useTasks.ts                      # 通用任务管理 Hook
├── components/
│   ├── TaskPanel/
│   │   ├── TaskTable.tsx                  # 通用任务表格
...
│       └── index.tsx                      # 导出
└── pages/
    ├── UnifiedTaskCenter.tsx              # 完整示例页面
    └── DataCenter/
        ├── index.unified.tsx              # 迁移后的 DataCenter
        └── index.components.ts            # 组件导出

扩展层（各任务独有）:
├── SyncTaskDrawer/                         # Sync 抽屉（待拆分）
│   ├── VisualEditorTab.tsx                 # Sync 可视化编辑
│   └── DataInspectTab.tsx                  # Sync 数据探查
└── ETLTaskDrawer/                          # ETL 抽屉（待拆分）
...
- **通用接口** `TaskTypeConfig<TTask, TStatus, TRunParams>`
  - 任务类型元数据（type, label, icon）
  - API 接口配置
  - 表格列配置
  - 操作按钮配置
  - 批量操作配置
...
- **工厂函数**
  - `createSyncTaskConfig(actions)` - 创建 Sync 任务配置
  - `createEtlTaskConfig(actions)` - 创建 ETL 任务配置
...
#### 4. 示例页面 (`pages/UnifiedTaskCenter.tsx`)
- 完整的统一任务管理示例
- Sync 任务 + ETL 任务统一管理
- 所有模态框和抽屉的集成
- 可直接运行查看效果

#### 5. 迁移后的 DataCenter (`pages/DataCenter/index.unified.tsx`)
- 使用新架构的完整 DataCenter
...
#### 1. 基础抽屉 (`components/TaskDrawer/BaseTaskDrawer.tsx`)
- 统一的抽屉壳
- 标题、关闭按钮
- 底部操作栏（取消/保存）
- 支持额外操作按钮
...
#### 2. 通用 Tab 组件 (`components/TaskDrawer/tabs/`)
