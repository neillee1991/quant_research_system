...
import { Button, Space, Tag, Collapse, Alert } from 'antd';
import {
  CheckCircleOutlined,
  WarningOutlined,
  LoadingOutlined,
...
const StrategyCodeEditor: React.FC<StrategyCodeEditorProps> = ({
  ...
  if (compileResult) {
    const hasError = compileResult.errors && compileResult.errors.length > 0;
    ...
    collapseItems.push({
      ...
      children: hasError ? (
        <Alert type="error" showIcon message={compileResult.errors.map((e, i) => <div key={i}>{e}</div>)} />
      ) : (
